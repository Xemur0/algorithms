...
for i in range(n):
    name = input(f'\nВведите название {i + 1}й предприятия: ')
    profit = 0
    q = 1
    while q <= QUARTER:
        try:
...
print(f'Предприятия, с прибылью выше среднего значения: {len(prof_c)}')
for name in prof_c:
    print(name)
print(f'Предприятия, с прибылью ниже среднего значения: {len(unprof_c)}')
